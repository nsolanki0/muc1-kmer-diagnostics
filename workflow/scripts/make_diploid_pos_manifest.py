...
def find_negative_pair(negative_dir, sample):
    ...
    r1 = os.path.join(
        negative_dir,
        f"{sample}_chr1_read1.fq.gz"
    )

    r2 = os.path.join(
        negative_dir,
        f"{sample}_chr1_read2.fq.gz"
    )
...
def add_pair(outfile, seen, pos_r1, pos_r2, neg_r1, neg_r2):
    ...
    pos_base = os.path.basename(pos_r1).replace(
        "_read1.fq.gz", ""
    )

    neg_base = os.path.basename(neg_r1).replace(
        "_read1.fq.gz", ""
    )
...
def main():
    ...
    with open(args.output, "w") as outfile:
        ...
        outfile.write(
            "Pair_ID\tPOS_R1\tNEG_R1\tPOS_R2\tNEG_R2\n"
        )

        # POS_MAT × NEG_PAT
        for pos_sample in pos_mat:

            pos_r1, pos_r2 = find_positive_pair(
                args.positive_dir,
                pos_sample
            )
            ...
            for neg_sample in neg_pat:
                ...
                neg_r1, neg_r2 = find_negative_pair(
                    args.negative_dir,
                    neg_sample
                )
