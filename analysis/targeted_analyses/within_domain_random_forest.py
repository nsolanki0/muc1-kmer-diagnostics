...
#    cv_results = cross_validate(pipeline, X_train, y_train, cv=skf, scoring=scoring, n_jobs=-1) # parallelize cv
    cv_stats = {}
...
results_df = pd.DataFrame(results)
results_df.to_csv(os.path.join(RES_DIR, "s4_rf_cv_feature_selection_val_macro.csv"), index=False)
summary_df = pd.DataFrame(summary_results)
summary_df.to_csv(os.path.join(RES_DIR, "s4_rf_cv_summary_feature_selection_val_macro.csv"), index=False)
...
plt.tight_layout()
plt.savefig(os.path.join(RES_DIR, "s4_rf_cv_feature_selection_all_metrics.png"), dpi=300)
plt.show()
plt.close()
...
y_test_prob = final_pipeline.predict_proba(X_test)[:, 1]  # Probability of positive class
...
report_df_final['Model'] = 'Random_Forest'
cols = report_df_final.columns.tolist()
cols = cols[-1:] + cols[:-1]
report_df_final = report_df_final[cols]
report_df_final.loc['best_n'] = best_n
report_df_final.to_csv(os.path.join(RES_DIR, f"s5_rf_feature_selection_{best_n}_classification_report_test.csv"), index=True)
...
plt.savefig(os.path.join(RES_DIR, f"s5_rf_feature_selection_{best_n}_confMat_test.png"), dpi=300, bbox_inches="tight")
plt.show()
plt.close()
...
feature_imp_df.to_csv(os.path.join(RES_DIR, f"s5_rf_selected_features_importances_{best_n}.csv"), index=False)
...
prediction_results.to_csv(os.path.join(RES_DIR, f"s5_rf_test_sample_predictions_{best_n}.csv"), index=False)


# ====================== 6. FEATURE ANALYSIS (HEATMAP) ======================
# 1. Extract selected features and model importances
selector = final_pipeline.named_steps["selector"]
final_classifier = final_pipeline.named_steps["classifier"]
...
top_25 = feature_imp_df.head(25)
bottom_25 = feature_imp_df.tail(25)
selected_features_df = pd.concat([top_25, bottom_25])
...
plt.ylabel("Features ordered by Random Forest importance", fontsize=15)
plt.title("Heatmap of Top 25 and Bottom 25 Features by Importance", fontsize=20)
...
plt.tight_layout()
plt.savefig(os.path.join(RES_DIR, "s6_rf_feature_heatmap_top25_bottom25.png"), dpi=300, bbox_inches="tight")
plt.show()
plt.close()
